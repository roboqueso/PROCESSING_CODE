...
void draw() {
    ...
    a = x - int( cos(radians(frameCount)) * alf );
    b = y - int( sin(radians(frameCount)) * alf );
...
dots();
 ...
 /*
     ranPalStroke(p1);
     ellipse( x, y, alf/2, alf/2 );
     ellipse( y, x, alf/2, alf/2 );

    stroke(random(frameCount/alf/TWO_PI), random(255),  random(alf/frameCount-PI), alf);
     ellipse( a, b, alf, alf );
     ellipse( b, a, alf, alf );
*/ 
...
float safeIt( float n ) {
  ...
  drawLissajous( n, random(n), frameCount/n );
...
void artDaily( String dailyMsg ) {
  ...
  fill(#EF2012);
  text( " "+dailyMsg, width-dailyMsg.length(), height);
  /*
float yy = 0;
   while( yy <= height ) {
