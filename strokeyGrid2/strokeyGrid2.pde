...
//import javafx.application.Platform;  //  if FX2D
int sColor = 0;
float x, y;
int sz = 1000;
FixLib fix;
...
void settings(){
  size(displayWidth, displayHeight, FX2D );//, P3D);  //, FX2D );  //P3D);
  pixelDensity(displayDensity());
  smooth(8);
  //Platform.setImplicitExit(false);  //  if FX2D
}
void setup()
{
  background(#444444);
...
theta = sqrt(frameCount%displayWidth) * 2 * PI;
r = sqrt(frameCount%displayWidth);
xx = cX -cos(sqrt(frameCount%displayWidth)*2*PI)*sqrt(frameCount%displayWidth);
yy = cY - sin(sqrt(frameCount%displayWidth)*2*PI)*sqrt(frameCount%displayWidth);
...
if(frameCount%r==0)
{
  ...
  strokeWeight(sqrt(sz));
    ...
    cX = random(displayWidth/2);
    cY = random(displayHeight/2);
  }
...
class FixLib {
  ...
  public void bitHeart( float x, float y, boolean grid ) {
    ...
    if (grid) {
      ...
      for ( int xx = 0 ; xx <= 13; xx++ ) {
        ...
        line( x+(blockSize*xx), 0, x+(blockSize*xx), displayHeight );
        line( 0, y+(blockSize*xx), displayWidth, y+(blockSize*xx) );
      }
    }
    //  GRID
  ...
  public void star(int n, float cx, float cy, float w, float h, float startAngle, float proportion)
  {
    if (n > 2)
    {
      ...
      float dw; // draw displayWidth
      float dh; // draw displayHeight
  ...
  public float getMax( float shapeSize ) {
    return ( ( displayWidth * displayHeight ) / shapeSize );
  }
  ...
  public void paletteGrid( ArrayList pall ) {
    ...
    fill(random(alf));
    text( pall.size(), random( alf, displayWidth/3 ), random(displayHeight) );
  }
  ...
  //////////////////////////////////////////////////////////////////////////
  //  Draw manual circle
  //  ellipse(x, y, displayWidth, displayHeight)
