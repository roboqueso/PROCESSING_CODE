...
public class MINIM_mp3_backgrounder extends PApplet {
...
public void setup()
{
    ...
    //  smooth() can only be used in settings();
...
public void draw()
{
  ...
  if(bgFrame!=null && frameCount % 4 == 0){
    //background(bgFrame);
    tint(255, PApplet.parseInt(21 * (1 + player.left.level() + player.right.level()) ));
    image(bgFrame,0,0);
  ...
  //  EX: 0.022453465
  radSz = PApplet.parseInt(11 * (1 + player.left.level() + player.right.level()) );
  z = (frameCount%radSz);
...
//drawLissajous( x, y, ( radSz + player.left.level() + player.right.level()) );
  ...
  for(int i = 0; i < bfSz - 1; i++)
  {
    ...
    cirX    = PApplet.parseInt(cx - PApplet.parseInt( cos(radians(cirAng)) * (cirSz + (player.left.level()*cx) ) ) );
    cirY    = PApplet.parseInt(cy - PApplet.parseInt( sin(radians(cirAng)) * (cirSz + (player.right.level()*cy) ) ) );
    //  hack safety
    cirX = cirX%width;
    cirY = cirY%height;
  ...
  strokeWeight(.75f+player.left.level()+player.right.level());
  noFill();  
...
public void keyPressed()
{
  ...
  public void settings() {  size(1280, 720, "processing.opengl.PGraphics3D");  smooth(8);  pixelDensity(displayDensity()); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "MINIM_mp3_backgrounder" };
    if (passedArgs != null) {
