...
{
	size(1440, 1080, P3D);
	H.init(this).background(#FFFFFF).autoClear(false).use3D(true);
	smooth();
	...
	r1.strokeWeight(1).stroke(#efefef).noFill().loc(0,0);
	rects.add(r1);
	...
	r2.strokeWeight(1).stroke(#efefef).noFill().loc(colW+15,0);
	rects.add(r2);
	...
	r3.strokeWeight(1).stroke(#efefef).noFill().loc(colW*2+30,0);
	rects.add(r3);
	...
	r4.strokeWeight(1).stroke(#efefef).noFill().loc(colW*3+45,0);
	rects.add(r4);
	...
	r5.strokeWeight(1).stroke(#efefef).noFill().loc(colW*4+60,0);
	rects.add(r5);
...
// stripes under boxes
for(int yy = 0; yy <= height; yy += random((int)1,TWO_PI))
{
	strokeWeight(.5);
	stroke(random(200,255),5);
	line(random(width), finger_tip.y, random(width), finger_tip.y);
}
