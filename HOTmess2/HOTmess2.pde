...
void draw() {
  ...
  noiseDetail( int(random(8)) );
    ...
    noiseFloat = noise(x * 0.0025, y * 0.0025, frameCount * 0.001); 
 
    a = x - int( cos(radians(frameCount)) * noiseFloat )+2;
    b = y - int( sin(radians(frameCount)) * noiseFloat )+2;

    x += noise(x,y,a)+cos(((noiseFloat -0.3) * TWO_PI) * 10)+2;
    y += random(noise(y,x,b))+sin(((noiseFloat - 0.3) * TWO_PI) * 10)+2;
...
void safeIt( float n ) {
  ...
  ranPalStroke(p3);
  hexagon( n, n, frameCount );
  radialLine(n,frameCount,n,frameCount);  
  dots();
  drawLissajous( b, y, random(n) );

    ranPalStroke(p1);
    drawLissajous( a, x, frameCount );    
}
...
void artDaily( String dailyMsg ) {
  ...
  fill(#EFEFEF);
  text( " "+dailyMsg, width-224, height);
  /*
float yy = 0;
   while( yy <= height ) {
