...
void draw()
{
  ...
  squareGrid( xx, yy, shapeSize+ct, alf+ct );
...
void squareGrid( float x, float y, float shapeSize, float pad ) {
  ...
  for( int ct = 0; ct < mCt; ct++ ) {
    
    noFill();
    smooth();
    
    rect( xx, yy, shapeSize, shapeSize, ct );
    ...
    if( ct % ( width+height ) == 0 ) {
      shapeSize = nextFib( int(ct) );
    }

   
   if( yy >= 768 ){
     stroke(random(-xx,xx), random(-yy,yy), pad, 11);
     strokeWeight(1);
     rect( random(1024), random(768), pad, pad, pad );
     break;
   }
...
void doExit() 
{
  ...
  {
    save( pdeName() + "-" + 
      month()+""+day()+""+year()+""+hour()+""+minute()+""+second()+""+millis()+".png" );
  }
...
void artDaily( String dailyMsg ) {
  ...
  text( dailyMsg, (1024/2)*1.2, 24 );  // 28
