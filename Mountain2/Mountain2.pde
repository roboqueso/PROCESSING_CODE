...
//  line( xx2, yy3, xx4, yy5 );
  ...
  //  tall oval
  //point( xx4, yy5 );
  ellipse( xx4, yy5, ( angle4/radians(radius4) ), ( angle4/radians(radius4) ) );
  stroke(0,alf);
  ellipse( xx5, yy1, ( angle3/radians(radius3) ), ( angle3/radians(radius3) ) );
  
  strokeWeight(.2);
  ranPalStroke( p2 );
  if( angle1 % 6 == 0 ) {
    hexagon( xx4, yy5, random(alf)*2 );
  } else {
    circle( xx4, yy5, random(alf), random(alf) );
  }

  // wide oval
  strokeWeight( 0.303 );
  ranPalStroke( p2 );  // stroke(#402014, alf*4);  //  stroke(#A10000, alf);
  //point( xx5, yy1 );
  ellipse( xx3, yy4, radians(angle5/alf)+(PI*radius5), radians(angle5/alf)+(PI*radius5) );
