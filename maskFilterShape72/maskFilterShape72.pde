...
textSize(sz);
text(frameCount, xx,yy, frameCount);
text(frameCount, yy,xx, frameCount);
textSize(sz/PI);
text(frameCount, xx,yy, frameCount);
text(frameCount, yy,xx, frameCount);
	...
	shape(shp, (xx*sz)%width,yy,getSeven(sz,xx), getSeven(sz,yy));


    //	one w/flavor
    rotate(QUARTER_PI/4, 1, 1, 0);
    filter(INVERT);
    tint(getSeven(sz,xx),getSeven(sz,yy), getSeven(xx, width));
