...
//  stroke(#75EF91, alf*4);
randStroke();
  hexagon( xx, yy, random(radius) );
//  stroke(#EF7519, alf*4);
  hexagon( outerXX, outerYY, random(outerRadius) );
...
{
  // pallete
  stroke( palette[ int(random( palette.length-1 )) ] , alf*1.5 );
}
void randFillUser()
{
  fill( palette[ int(random( palette.length-1 )) ] , alf*1.5 );
}
