...
PImage img, img2;
ArrayList p3;
float a, b, amp = 3, x, y, r, g, noiseFloat;

////////////////////////////////////////////////////
//
void setup() {
  // setup core sketch settings items
  size(1024, 768, OPENGL);  //  P3D  OPENGL
  background(9);
	frameRate(666);
  //  setup variables
  noFill();
...
//    noiseSeed( int(random(TWO_PI)) );
    noiseDetail( int(random(frameCount)) );  //  a


    x += random(noise(x)) + (alf/100);
    y += random(noise(y)) + (alf/100);
 
    a = noise(x) + int( cos(radians(frameCount/PI)) * (alf*TWO_PI) ) - (alf/100);
    b = y + int( sin(radians(frameCount/PI)) * (alf*TWO_PI) ) - (alf/100);
...
//    MAKE SAFE
    ...
    stroke( random(255), random(frameCount/alf/TWO_PI), random(alf/frameCount-PI), alf*4.2); 
    strokeWeight(random(alf/PI));
    dots();
    ellipse( x, y , alf, alf );
    ellipse( y, x , alf, alf );
...
/////////////////////////////////////////////////////////////////////////////
//  Keep var within width x height and throw in some spice
void safeIt( float n ) {
  n = random(alf);
  ...
  ranPalStroke(p3);
  radialLine(alf,frameCount,alf,frameCount);  
  dots();
...
//  stroke( random(255), random(frameCount/alf/TWO_PI), random(alf/frameCount-PI), alf*4.2);
  ...
  a = x + int( cos(radians(frameCount/PI)) * (alf*TWO_PI) );
  b = y + int( sin(radians(frameCount/PI)) * (alf*TWO_PI) );
  ranPalStroke(p3);
  dots();
...
//  float amp = 33;
  ...
  {
    x = noise( a - amp * sin(a * t * PI/180) );
    y = noise( b - amp * sin(b * t * PI/180) );
