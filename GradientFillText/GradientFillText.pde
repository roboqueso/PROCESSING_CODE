...
TODO:
- correct color pallete to match photo
...
int strokeSz = 7;  //  Stroke weight affects the FRAME and the TEXT
...
void setup() 
{
  ...
  //  fill
  gradient = LinearGradient.valueOf("linear-gradient(from 0px 0px to 0px "+height+"px, #EF1975 25%, #13EF13 50%, #8833EF 75% )");
  //  stroke
  gradStroke = LinearGradient.valueOf("linear-gradient(from 0px 0px to 0px "+height+"px, #EFEFEF 25%, #EF4318 50%, #242424 75%)");
...
void draw() {
    ...
    //  "-bMode-" + bMode + 
    SAVE_NAME = fix.pdeName() + "-" + (frameCount-1) + "-" + fnt  + "-strokeSz-" + strokeSz ;
