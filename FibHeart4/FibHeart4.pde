...
point( yy, xx );
 ...
 //   point( lerp(xx, yy, .5), lerp(outerXX, outerYY, .5) );
 //   point( lerp(outerXX, outerYY, .5), lerp(xx, yy, .5) );
...
//  circle( xx, yy, alf, alf );
}

outerRadius += alf;
if( outerRadius > height || outerRadius < 0 ) {
  outerRadius = alf;
  circle( outerXX, outerYY, alf, alf );
}
/*    
    radius --;
    outerRadius++;
    ...
    if( radius > height || radius < 0 ) {
      ...
      radius = ( nextFib( int(radius) ) < height ) ? nextFib( int(radius) ) : alf;
      outerRadius = ( nextFib( int(outerRadius) ) < height ) ? nextFib( int(outerRadius) ) : alf;
    }
  */
