...
void setup() {
  ...
  t = angle = 0;
    ...
    // fat stripes ------------------------------------
    strokeWeight(w);
    for( float h = -w; h < width; h += w+2 ) {

      stroke(#A2A2A2, 22);
      line( h, 0, h, height );

      ranPalStroke(p1);
      drawLissajous( h, cY, random(h) );
      ranPalStroke(p2);
      drawLissajous( cY, h, random(h) );
...
void draw()
{
  xx = startX - int( cos(radians(angle)) * random(w) );
  yy = startY - int( sin(radians(angle)) * random(w) );
  ...
  ranPalStroke( p1 );
  drawLissajous( yy, xx, random(frameCount) );
...
void artDaily( String dailyMsg ) {
  ...
  fill(#EFEFEF);
  text( " "+dailyMsg, this.width-228, this.height-14);
/*
float yy = 0;
while( yy <= height ) {
