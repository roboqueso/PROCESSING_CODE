...
//  UPDATE IMAGE PIXELS
  ...
  for( int c = 0; c < 36; c ++ ) {
    loc = int( c+px + (py * width) );
    pixels[ constrain( loc, 0, pixels.length-1 ) ] =  pixels[ constrain( loc, 0, pixels.length-1 ) ] + 50;
  }
  updatePixels();// int(px), int(py), 36, 36);
//  UPDATE IMAGE PIXELS
...
////////////////////////////////////////////////////////////////
  ...
  //  Team Fickes
textFont( loadFont( "AnonymousPro-Bold-14.vlw") );
//textFont( createFont( "HelveticaNeue", 14 ) );

  fill( #ABABAB );
  text( "TEAM FICKES", cX+78, cY-125 );
  
  fill( #EFEFEF );
  text( "TEAM FICKES", cX+79, cY-126 );
...
color getRanColor(color[] palette)
{
  // pallete
  return palette[ int(random( palette.length-1 )) ];
}
/*
//  get random color from palette
int getRanColor(ArrayList palette)
{
  // pallete
  return (int)palette.get( (int)random( palette.size()-1 ) );
}
*/
////////////////////////////////////////////////////
