...
void draw()
{
  ...
  px = sin(cX)*alf + cos(radians(angle))*(radius/2);
  py = sin(cY)*alf + sin(radians(angle))*(radius/2);


  strokeWeight(.5);
  stroke(#EFEFEF, alf);
  rect( cX, cY, angle-alf, angle-alf );
  ...
  strokeWeight(2);
  ranPalStroke(p2);
  ellipse( px, py, alf, alf );

  //  
  ranPalStroke(p1);
  //  triangle( alf+frameCount, angle, ct, cX, cY, shapeSz );
  curve( alf+frameCount, alf+angle, alf+ct, shapeSz-cY, shapeSz-cX, shapeSz, alf+angle, shapeSz-cX/ct );


  stroke(#006768, alf );
  curve( alf+frameCount, angle, ct, cY, cX, shapeSz, alf+angle, cX/ct );


  ranPalStroke(p1);
  arc( cX, alf, shapeSz, ct, frameCount, radius );
