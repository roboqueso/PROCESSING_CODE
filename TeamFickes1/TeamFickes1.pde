...
if(flip) {
 ii += shapeSize; 
  ...
  textFont( createFont( "Futura-Medium", 100 ) );
  smooth();

  fill(0);
  text( "team fickes", 10, this.height*.75);
...
///////////////////////////////////////////////////////
//  draw grid filled with each color from supplied ArrayList
...
//text( pall.size() + " colors ", sz, sz );
  ...
  for( int cc = 0; cc < pall.size(); cc++ ) {
    ...
    fill(random(alf), alf*TWO_PI);
    text( "ERICFICKES.COM", random( alf, width/3 ), random(height) );
