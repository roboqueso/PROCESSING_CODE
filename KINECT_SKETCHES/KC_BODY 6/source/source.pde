...
imgDepth.filter(GRAY);
// imgIr.mask(imgDepth);
imgDepth.mask(imgIr);

tint(frameCount%255,frameCount%255,frameCount%255);
image(imgDepth, 0, frameCount%height, width, imgDepth.height);
// image(imgIr, 0, frameCount%height, width, imgDepth.height);
  ...
  // draw the skeleton if it's available
      ...
      // if(frameCount%90==0){
        ...
        stroke(tmp.x%255,tmp.y%255, tmp.z%255);
  	    ...
  	    beginShape(TRIANGLES);//QUADS);  //   POLYGON, TRIANGLES, QUADS, or QUAD_STRIP
