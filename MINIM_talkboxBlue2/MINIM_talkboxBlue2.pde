...
//  ** X adjusted by cX+  
    ...
    line( cX+  i+1, cY + in.right.get(i+1)*frameCount,i, cY + in.right.get(i)*frameCount );
    //  OG WAVEFORM
    
    
    
    strokeWeight(.75);
    stroke(37, random(255),37,  alf/2);
  ...
  //  RIGHT
  text( in.right.get((int)random(in.bufferSize() - 1)), width-alf-szW*szH*noise(frameCount), random(height) );
  noFill();
  ...
  fill(random(11),random(37),random(11), 1);
  ellipse( cX, cY, m, m );
...
//    fix.randFill();
      ...
      stroke(b,r,g,alf);
//      fill(r,g,b,alf);
    stroke(frameCount%2==0?#2012EF:#1975EF, alf);
