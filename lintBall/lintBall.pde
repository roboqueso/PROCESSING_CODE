...
void draw() {
  ...
  x = (cX-radians(cX + amp) * cos(a * frameCount * PI/180) % width );
  y = (cY-radians(cY + amp) * sin(b * frameCount * PI/180) % height );
println( frameCount + " " + x + " " + y );
  fix.ranPalStroke100(p1);
  strokeWeight(noise(alf));
  fix.radialLine( a,y,b,x );
  ...
  if ( frameCount % 90 == 0 )
  {
    ...
    fix.ranPalStroke(p1);
    strokeWeight(1);
    fix.radialLine( a, b, x, y );
    fix.radialLine( y, a, b, x );
