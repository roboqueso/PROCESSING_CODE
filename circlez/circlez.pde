//  SEE:   https://ello.co/ericfickes/post/zvsoswnfolwqp7eybngcsw
...
stroke( random(alf/PI), random(alf*PI), random(255), alf/PI);
...
//	strokeWeight()
strokeWeight(TWO_PI+(angle*noise(angle)) );
//	strokeCap()
...
strokeWeight(random(radius));
stroke(random(255));
point( width-y, height-x );
point( y, x);


translate( (cX+x)%width , (cY+y)%height, alf);

fill(random(255),random(255),alf, alf*TWO_PI);
stroke(#2012EF, random(alf/PI));
